...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	bindCompositePoolAnthropicProtocolFilter(c, apiKey)
	...
	if platform == service.PlatformGemini {
		fs := NewFailoverState(capAccountAttemptsForGroup(apiKey, h.maxAccountSwitchesGemini), hasBoundSession)
	...
	for {
		fs := NewFailoverState(capAccountAttemptsForGroup(currentAPIKey, h.maxAccountSwitches), hasBoundSession)

...
// qwCompact 千问办公的顺序 JSON：version,requestId,info,cosyVersion,ideVersion。
// 与 GUI generateAuthToken 的字段顺序一致，服务端按序校验。
func qwCompact(p map[string]string) string {
	return "{" +
		"\"version\":\"" + p["version"] + "\"," +
		"\"requestId\":\"" + p["requestId"] + "\"," +
		"\"info\":\"" + p["info"] + "\"," +
		"\"cosyVersion\":\"" + p["cosyVersion"] + "\"," +
		"\"ideVersion\":\"" + p["ideVersion"] + "\"" +
		"}"
}
...
func (s *CosySession) AuthHeader(body, rawURL, uid string) (string, error) {
	payload := map[string]string{
		...
		"ideVersion":  "1.0.0",
		"info":        s.Info,
		"requestId":   uuidNoDash(),
		"version":     "v1",
	}
	payloadB64 := base64.StdEncoding.EncodeToString([]byte(qwCompact(payload)))
...
// uuidNoDash 无横线 UUID（千问办公 requestId/machine 用，对齐 GUI randomUUID().replace(/-/g,"")）。
func uuidNoDash() string {
	return strings.ReplaceAll(uuid4(), "-", "")
}
...
// UUIDNoDash 对外暴露无横线 UUID。
func UUIDNoDash() string { return uuidNoDash() }

...
//
// parseNestedSSE 逐行解析 SSE，双格式兼容：
//   - 嵌套格式（qoder）：data:{"body":"<json-string>"}，body 二次解析
//   - 标准格式（千问办公）：data:{"choices":[{"delta":{...}}]}
//   - body == "[DONE]" 或 data: [DONE] → 正常结束
	...
	for {
		...
		if strings.HasPrefix(line, "data:") {
			...
			if json.Unmarshal([]byte(payload), &env) == nil && env.Body != "" {
				...
				if json.Unmarshal([]byte(body), &chunk) == nil {
			...
			// 2. 标准 OpenAI SSE 格式：data:{"choices":[...]}
			var chunk map[string]any
...
// 上游（qwenwork/qoder）流式 chunk 的 finish_reason 恒为 ""（空串），若原样
// 透传，客户端会误判「流已结束」而中断，只收 1 个分片。这里把空串 finish_reason
// 规范化为 null，非空（如 "stop"）保留——中间片为 null、末片才有结束原因。

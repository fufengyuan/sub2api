...
func (c *Client) fetchModels(a *auth.Auth) ([]DynamicModel, error) {
	...
	// 千问办公模型在 qwork 场景（qoder 在 chat 场景）：优先 qwork，回退 chat。
	var sceneRaw json.RawMessage
	var sceneName string
	if v, ok := apiResp["qwork"]; ok && len(v) > 0 && string(v) != "null" {
		sceneRaw, sceneName = v, "qwork"
	} else {
		sceneRaw, sceneName = apiResp["chat"], "chat"
	}
	if len(sceneRaw) == 0 {
		return nil, fmt.Errorf("no qwork/chat scene in models response")
	}
	var models []DynamicModel
	if err := json.Unmarshal(sceneRaw, &models); err != nil {
		return nil, fmt.Errorf("%s scene parse: %w", sceneName, err)
	}

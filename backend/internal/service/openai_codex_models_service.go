...
func (s *OpenAIGatewayService) BuildGroupConfiguredCodexModelsManifest(
	...
	body, err := buildCodexModelsManifestForAccounts(PlatformOpenAI, configuredModels, catalog)
	if err != nil {
...
func (s *GatewayService) BuildCodexModelsManifestForGroup(
	...
	// composite 统一账号池没有分组级路由表：Codex 元数据一律由组内账号的
	// model_mapping 别名声明与内置模型探测推导。
	return buildCodexModelsManifestForAccounts(effectivePlatform, modelIDs, catalog)
}

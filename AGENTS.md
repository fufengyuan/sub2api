...
- **三渠道积分/签到/模型映射**: 管理端账号级 API `POST /:id/credits/refresh`、`GET /:id/credits/detail`、`POST /:id/checkin`、`GET /:id/upstream-models`（`CnUpstreamHandler`，签到业务失败返回 success=false 而非 error）；余额落 credentials `creditsRemain`；账号级模型映射存 credentials `model_mapping`（转发经 `GetMappedModel` 生效）；自动签到由 config.yaml `checkin.enabled` 开启（调度器随 Wire Provider 自启）。详见 [docs/cnupstream-credits-checkin-design.md](docs/cnupstream-credits-checkin-design.md)。

...
- **计费缺价（`no pricing available`）与国产渠道积分折算**: 上游模型（尤其 CN 渠道动态模型如 `qwen3.8-flash`）在 LiteLLM 价卡与 `fallbackPrices` 里没条目时，`calculateTokenCost` 会失败。这类**不是系统故障**——请求已成功，只是缺价，**必须零成本落账 + WARN**（旧路径 `gateway_usage_billing.go` 与新路径 `openai_gateway_usage.go` 共用 `isUsagePricingUnavailableError`，定义在 `usage_pricing_shared.go`），绝不刷 ERROR 淹没真实故障、也不丢弃 usage。CN 四渠道可按上游积分倍率补齐计费：`FetchModelPricing` 返回的 `Rate`（traework `consumption_rate.rate` / qoder+qwenwork `price_factor` / workbuddy `credits` 字段）是**每次请求的积分消耗量**（非每 token 单价，量纲不同，不要塞进 `fallbackPrices`），折算见 `cn_credit_pricing.go`：成本 = `Rate × CreditToUSD × multiplier`，配置 `cn_credit_pricing.{enabled,credit_to_usd}` **默认关闭**（关时行为与改动前一致，不误扣）；倍率表未收录的模型按 `cnCreditPricingDefaultRate=1` 保守折算（非零成本不白嫖、不猜高价不误扣）并打 WARN；倍率缓存按**平台+账号**隔离（同平台不同账号套餐折扣可能不同，只按平台缓存会串号）。详见 [docs/cn-credit-pricing.md](docs/cn-credit-pricing.md)。

- **推理字段透出（reasoning\_content / thinking tokens）**: 网关**所有链路响应侧原样透传，不剥 reasoning\_content、不清 usage thinking 字段**——客户端收到 reasoning\_content 恒空或 thinking tokens=0，根因在上游（上游没返回思考内容或字段名非 `reasoning_content`），不是网关剥字段。请求侧：qwenwork/qoder 的 `buildAgentBody` 重构精简 body 时会**丢弃客户端** **`reasoning_effort`** **强度值**（已修复为写入 `model_config.reasoning_effort`，2026-09-02，提交 `dfc0fff63`）且**不把** **`thinking`** **字段本身传给上游**（只转 `model_config.is_reasoning` 布尔）；traework/workbuddy 为浅改写透传，thinking/reasoning\_effort 原样转发。详见 [docs/cn-upstream-reasoning-content-transparency.md](docs/cn-upstream-reasoning-content-transparency.md)。
...
- **CN 渠道流式 SSE 必须符合 OpenAI chunk 规范**（否则 zcode 等客户端把 reasoning\_content 切成"一段一段思考"）。规范基线取自项目内 `internal/pkg/apicompat` 的 `resToChatHandleCompleted` / `makeChatFinishChunk`，四条硬约束：① `finish_reason` 字段**恒定存在**——末 chunk 为具体原因（`stop`/`length`/`tool_calls`/…），中间 chunk 为 `null`，**绝不能缺字段或给空串**（缺字段时客户端退化为按 chunk 到达时延/字段切换切分 UI 块）；② **首 chunk 必须带** **`delta.role="assistant"`**，且 role 只出现一次（中间/末 chunk 不带）；③ **`reasoning_content`** **与** **`content`** **不得同处一个 delta**（上游混发时必须拆成两个 chunk，先思考后正文）；④ **`usage`** **是 finish chunk 之后的独立 chunk 且** **`choices`** **为** **`[]`**（挂在 finish chunk 上会让客户端收到 finish\_reason 就停止解析而丢 usage，表现为 token 恒为 0）。四渠道均已对齐：traework 由 `streamOpts.writeChunk`（`cnupstream/traework/solosse.go`）实现，用独立状态 `wroteAnyChunk` 判断首 chunk——**不可复用** **`sawOutput`**（后者语义是"上游产出过内容"，被 `case "error"` 用于决定是否返回 `StreamBeforeOutputError` 让上层换号重试）；上游无 output 直接 done 时末 chunk 同时是首 chunk，仍须带 role。qoder/qwenwork 由重写后的 `streamAsOpenAI` 实现（注意 qoder 的 `parseNestedSSE` **只认嵌套格式** `data:{"body":"<json>"}`，qwenwork 兼容嵌套+标准两种）。workbuddy（upstream）由 `emitNormalized` 逐行解析规范化，**非 JSON 行（注释行/event: 行/\[DONE]）仍原样透传**。非流式由 `normalizeCnAggregateResponse` 补齐 `model`（traework 聚合恒为 `""`）、零值 `usage`、`choices[0].index/finish_reason/message.role`。详见 [.claude/artifacts/fixes/cn-traework-stream-openai-spec.md](.claude/artifacts/fixes/cn-traework-stream-openai-spec.md)。

- **TraeWork 1M 上下文开关 = llm_utils_chat 请求体的 `prompt_max_tokens` 字段（不是 `__max` 后缀）**。已连真实上游实测（2026-09-03，中转站 traework 账号）：① **绝不给 `config_name`/`model` 追加 `__max`/后缀**——会 4001 param is invalid（逆向渠道客户端遥测 `model_name:"...__max"` 是云端 agent 编排的会话模型变体标识 ≠ 上游 API 契约）；② 正确做法是完整对齐真实客户端 `llm_utils_chat` payload：`model_name=<名>__dev`（独立字段，非 config_name）、`prompt_max_tokens`、`max_tokens`（**尊重客户端传值**，max_completion_tokens 优先/其次 max_tokens，都没传默认 128K=131072——别照搬真实客户端写死的 4096，那是其免费档输出上限）、`conversation_id/session_id/project_id`(uuid)、`user_id/device_id/machine_id`(取自 auth.Auth)、`workspace_id/mode/ide_version/app_id`——补齐后 200 正常流；③ 实测 `prompt_max_tokens=1000000`（1M）上游 200 接受、正常流，sub2api `promptMaxTokens` 默认已设 1M，traework 账号可用大输入——**但注**：对照实测（[traework-llm-chat-field-align.md](.claude/artifacts/designs/traework-llm-chat-field-align.md) §3.2）显示 `prompt_max_tokens` **不是硬输入上限**（设 200 塞 4000 token 输入也正常输出不截断），它更接近客户端期望窗口声明，设 1M 是**安全乐观值**（不阻止大请求）但**不能证明 1M 真被启用**；而 `max_tokens`（输出上限）**确实生效**（20→1627 / 3000→4393 completion），故 respect 客户端传值、默认 128K。实现见 `traework/payload.go PrepareBody(src, a)`（**签名带 auth.Auth** 写身份字段）。验证路径：`real_upstream_integration_test.go`（-tags=integration，凭证走 env，从 DB 注入不落盘）。另：TraeWork 对话走自研 `ttnet` 不遵循 HTTP_PROXY，抓包需临时切系统代理到 mitmproxy。

...
- **TraeWork Max 1M 上下文 = 模型名加 `__max` 后缀**（经抓包确认：TraeWork 接口在 `api5-normal.mchost.guru`，配置层 model 为裸名 `DeepSeek-V4-Flash-Official`，实际执行的上游模型标识为 `DeepSeek-V4-Flash-Official__max`，且**无独立上下文控制字段**）。网关在 `forwardCnUpstreamChatCompletions` 对 **traework 平台**默认给上游 model 追加 `__max`（`ensureTraeWorkMaxSuffix`，幂等——客户端已带则不重复拼成 `xxx__max__max`），从而默认启用 1M；**计费模型名 `billingModel`/`originalModel` 保持裸名**（`__max` 仅上游识别上下文用，避免污染计费 key）。仅 traework 平台，qwenwork/qoder 无此机制。另注意 TraeWork 实际 LLM 请求体 `create_agent_task` **应用层加密**（mitmproxy 只能看 HTTP 层），对话请求经自研 `ttnet` 网络栈、**不遵循 HTTP_PROXY/--proxy-server**，只有临时切**系统代理**到 mitmproxy 才能抓到（需装 mitmproxy CA 到系统钥匙串）。详见 [.claude/artifacts/designs/traework-upstream-max-suffix.md](.claude/artifacts/designs/traework-upstream-max-suffix.md)。

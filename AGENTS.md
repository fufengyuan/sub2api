...
- **⚠️ TraeWork 的 Max 1M 上下文是云端会话能力，外部 SOLO chat API 拿不到——绝不能给 chat API 的 model 追加 `__max` 后缀**。教训（2026-09-03）：曾据 TraeWork 客户端遥测 `model_name:"DeepSeek-V4-Flash-Official__max"` 推断"Max=模型名加后缀"，实现在网关自动追加后，**生产全部 traework 请求报 4001 param is invalid**（同账号裸名可成功，A/B 干净对照）。真相：① `__max` 只是 TraeWork 云端 agent 编排层（`create_agent_task`，应用层加密）的**会话模型变体标识**，与 SOLO chat API 的 model 参数无关；② wild-work 协议参照仓库 `internal/traework/payload.go` 中 `model`/`config_name` 均为裸名、全仓库无 `__max` 处理；③ 1M 上下文由 TraeWork 云端编排层按会话控制，外部聚合网关无法启用。**逆向渠道的客户端遥测标识 ≠ 上游 API 契约**，改协议字段前必须先对照 wild-work 参照实现。另：TraeWork 对话走自研 `ttnet` 网络栈、不遵循 HTTP_PROXY/--proxy-server，只有临时切**系统代理**到 mitmproxy 才能抓到（需装 CA 到系统钥匙串）。

...
- **中国マルチチャネル上流** - workbuddy / traework / qoder / qwenwork のサブスクリプションアカウントをネイティブサポート：ワンクリック OAuth 認証、クレジット照会と自動チェックイン、アカウント単位のモデルマッピング、統一プールスケジューリング（[設計メモ](docs/platform-multi-channel-integration.md)）
...
- **Composite 統一アカウントプール** - クロスプラットフォーム統一アカウントプール：同一グループ内でプラットフォームを横断したアカウントを統一スケジューリングし、プラットフォーム別ルーティングテーブルなし、エンドポイント能力はプール構成で決定（[操作ガイド](docs/COMPOSITE_GROUPS.md)）
...
│   │   ├── config/           # 設定
│   │   ├── model/            # データモデル
│   │   ├── service/          # ビジネスロジック（ゲートウェイ転送、課金、スケジューリング）
│   │   ├── handler/          # HTTP ハンドラー
│   │   ├── repository/       # データアクセス（Ent + Redis キャッシュ）
│   │   └── cnupstream/       # 中国マルチチャネル上流クライアント（workbuddy/traework/qoder/qwenwork）
│   └── resources/            # 静的リソース
│
